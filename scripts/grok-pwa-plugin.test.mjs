...
/**
 * An empty workspace. `injectGrokPwaHead` defaults `cwd` to process.cwd(), where
 * this repo's own `src/lib/og/site.json` ("Brutal Fist"), `public/og.jpg` and
 * `public/x-banner.jpg` would be picked up as the baked site identity. These
 * unit assertions are about the injector, not about Brutal Fist's branding, so
 * they read from a directory with none of it.
 */
const emptyWorkspace = () => mkdtempSync(join(tmpdir(), "grok-og-isolated-"));
...
test("platform chrome overwrites share-card metas and always sets og:title", () => {
  ...
  const out = injectGrokPwaHead(html, { appName: "Wild Race", cwd: emptyWorkspace(), site: {} });
...
test("streaming injector matches </HEAD> case-insensitively", () => {
  const injector = createHeadInjector({ appName: "Wild Race", cwd: emptyWorkspace(), site: {} });
  const chunks = [
