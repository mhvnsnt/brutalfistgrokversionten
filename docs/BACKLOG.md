...
| 1.16 | Some clips go THROUGH the floor | **mostly done** — a grounded clip is corrected both ways now; whole-set sink went 134 -> 45 clips. Of the 11 clips a fight is actually made of, 8 are within 3.4 cm and none sink. Residual: `attack_rk` (ROUNDHOUSEKICK) -25 cm, `attack_1` -4 cm. |
| 1.17 | 62 clips ask for more than 60 cm of floor correction | OPEN — mostly taunts (`TAUNT`, `CROTCHCHOP`, `RAPIDCHESTBEATING`, `TAU_*`) plus `FALLING_FLAT_IMPACT`. The shift is capped and they are named rather than smeared: a body that needs to move 1.45 m to reach the floor has a different defect, probably authored at another scale. |
